...
void radixCountingSort(Pokemon creatures[], int length) {
    Pokemon *output = (Pokemon *)malloc(length * sizeof(Pokemon));
    
    for (int i = 0; i < length; i++) {
        for (int j = 0; j < length - 1; j++) {
            if (compareFirstAbility(&creatures[j], &creatures[j + 1]) > 0) {
                Pokemon temp = creatures[j];
...
void radix(Pokemon creatures[], int tam) {
    radixCountingSort(creatures, tam);
}
...
int main(void)
{
    ...
    while (true)
    {
        ...
        int id = atoi(input);
        inputCreatures[i++] = pokemons[id];

    }

    radix(inputCreatures, i);

    for(int j =0;j<i;j++){
        printPokemon(&inputCreatures[j]);
    }
    clock_t end = clock();
    ...
    fprintf(arquivo, "844188\t96\t82\t%.2f\n",executionTime);
    fclose(arquivo);
