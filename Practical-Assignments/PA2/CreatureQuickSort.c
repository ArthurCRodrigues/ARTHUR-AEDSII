...
void quick(Pokemon *pokemons[], int esq, int dir) {
  if (esq < dir) {
    int i = esq, j = dir;
    Pokemon *pivo = pokemons[(esq + dir) / 2];

    while (i <= j) {
      while ((pokemons[i]->geracao < pivo->geracao) || (pokemons[i]->geracao == pivo->geracao && strcmp(pokemons[i]->nome, pivo->nome) < 0)) {
        i++;
      }
      while ((pokemons[j]->geracao > pivo->geracao) || (pokemons[j]->geracao == pivo->geracao && strcmp(pokemons[j]->nome, pivo->nome) > 0)) {
        j--;
      }
      if (i <= j) {
    ...
    quick(pokemons, esq, j);
    quick(pokemons, i, dir);
  }
}
...
int main(int argc, char **argv) {
  ...
  double executionTime = ((double)(end - start)) / CLOCKS_PER_SEC * 1000.0;
    ...
    FILE *arquivo = fopen("844188_quicksort.txt", "w");
    if (arquivo == NULL) {
        printf("Erro ao abrir o arquivo!\n");
        return 1;
    }

    fprintf(arquivo, "844188\t167\t86\t%.2f\n",executionTime);
    fclose(arquivo);
