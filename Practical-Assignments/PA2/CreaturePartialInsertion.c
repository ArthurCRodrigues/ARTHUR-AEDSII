...
// Função de ordenação por Insertion Sort, utilizando data de captura como chave e nome em caso de empate
void insertion_sort(Pokemon *pokemons[], int n) {
    for (int i = 1; i < n; i++) {
        Pokemon *temp = pokemons[i];
        int j = i - 1;

        // Compara com base na data de captura
        while (j >= 0 && 
            (pokemons[j]->data_captura.ano > temp->data_captura.ano ||
            (pokemons[j]->data_captura.ano == temp->data_captura.ano &&
            pokemons[j]->data_captura.mes > temp->data_captura.mes) ||
            (pokemons[j]->data_captura.ano == temp->data_captura.ano &&
            pokemons[j]->data_captura.mes == temp->data_captura.mes &&
            pokemons[j]->data_captura.dia > temp->data_captura.dia) ||
            // Em caso de empate na data, ordena pelo nome
            (pokemons[j]->data_captura.ano == temp->data_captura.ano &&
            pokemons[j]->data_captura.mes == temp->data_captura.mes &&
            pokemons[j]->data_captura.dia == temp->data_captura.dia &&
            strcmp(pokemons[j]->nome, temp->nome) > 0))) {
