...
struct indirectHash 
{
  ListaEncadeada **table;
  int size;
} typedef indirectHash;

indirectHash * createindirectHash() {
    indirectHash *tabela = (indirectHash *)malloc(sizeof(indirectHash));
    if (tabela == NULL) {
        printf("Memory allocation error\n");
        exit(1);
    }
    tabela->size = 21;
    tabela->table = (ListaEncadeada **)malloc(21 * sizeof(ListaEncadeada *));
    if (tabela->table == NULL) {
        printf("Memory allocation error\n");
        exit(1);
    }
    for (int i = 0; i < 21; i++) {
        tabela->table[i] = createListaEncadeada();
    }
    return tabela;
}
...
void pesquisarTabela(char *nome, indirectHash *table) {
  int pos = hash(nome);
  bool resp = pesquisarLista(nome, table->table[pos]);
  if (resp) {
...
void insert(char *nome, indirectHash *table) {
  int pos = hash(nome);
  inserirInicio(nome, table->table[pos]);
}
...
int main(void) {
  ...
  // Criar tabela hash
  indirectHash *tabela = createindirectHash();
  ...
  FILE *logFile;
  logFile = fopen("844188_hashIndireta.txt", "w");
  if (logFile == NULL) {
    printf("Erro ao criar arquivo de log\n");
    return 1;
  }
 
  fprintf(logFile, "844188\t%f\t%d\t%d\n", execution_time, comparacoes, movimentacoes);
  fclose(logFile);
