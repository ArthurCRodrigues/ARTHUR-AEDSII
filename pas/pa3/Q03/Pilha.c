...
Pilha inicializarPilha() {
    Pilha pilha;
    pilha.pokemons = malloc(100*sizeof(Pokemon*));
    pilha.n = 0;
    return pilha;
}

void inserir(Pilha *pilha, Pokemon *p) {
    pilha->pokemons[pilha->n] = p;
    pilha->n++;
}
...
void printPilha(Pilha *pilha) {
    for (int i = 0; i < pilha->n; i++) imprimirPokemon(pilha->pokemons[i]);
}
...
int main () {
    ...
    for (int i = 0 ; i < numOp ; i++) {
        ...
        if (strcmp(comandos, "R") == 0) {
            Pokemon* p = remover(&pilha);
            if (p != NULL) {
                printf("(R) %s\n", p->name);
            }
        }
