...
class PilhaPMON {
    CelulaPPMON topo;
    PilhaPMON() {
        topo = new CelulaPPMON();
    }
    public void inserir(Pokemon p) {
        CelulaPPMON nova = new CelulaPPMON(p);
        if (topo.prox == null) topo.prox = nova;
        else {
            nova.prox = topo.prox;
            topo.prox = nova;
        }
    ...
    public String remover() {
        CelulaPPMON removable = topo.prox;
        Pokemon resp = removable.pokemon;
        topo.prox = removable.prox;
        return "(R) "+resp.getName();
    }
    public void print() {
        print(topo.prox,this.tamanho()-1);
    }
    public int tamanho() {
        int x = 0;
        for (CelulaPPMON i = topo.prox; i != null; i = i.prox) x++;
        return x;
    }
    private void print(CelulaPPMON i, int n) {
        if (i == null) {
            return;
        }
        else {
            ...
            System.out.println(i.pokemon.imprimir());
        }
    }
}
public class MainPPMON {
    public static int processMethod(String input) {
        int method = -1;
        if (input.charAt(0) == 'I' ) method = 1;
        if (input.charAt(0) == 'R' ) method = 2;
        return method;
    }
    ...
    public static void main(String[] args) {
        try (Scanner inputScanner = new Scanner(System.in)) {
            ...
            PilhaPMON inputPokemons = new PilhaPMON();
            ...
            for (int i = 0 ; i < numOp; i++) {
                ...
                switch (method) {
                    ...
                    case 2:
                        try {
                            System.out.println(inputPokemons.remover()); 
                        } catch (Exception e) {
