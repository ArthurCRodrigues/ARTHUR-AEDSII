...
class PartialQuickSort {
    private Pokemon[] inputCreatures;
    private int tam;
    public int comp = 0;
    public int mov = 0;
    int k = 10;
    ...
    public Pokemon[] getPokemon() {
        return inputCreatures;
    }

    public PartialQuickSort(Pokemon[] inputCreatures, int tam) {
        this.inputCreatures = inputCreatures;
        this.tam = tam;
    }

    public void imprimirSort() {
        for (int i = 0 ; i < k ; i++) {
            System.out.println(inputCreatures[i].imprimir());
        }
    }
    ...
    private void quickSort(int low, int high) {
        int i = low;
        int j = high;
        Pokemon pivot = inputCreatures[(i+j)/2];

        while (i <= j) {
            while((inputCreatures[i].getGeneration() < pivot.getGeneration()) || ((inputCreatures[i].getGeneration() == pivot.getGeneration()) && (inputCreatures[i].getName().compareTo(pivot.getName()) < 0))) {
                i++;
                comp++;
            }

            while ((inputCreatures[j].getGeneration() > pivot.getGeneration()) || ((inputCreatures[j].getGeneration() == pivot.getGeneration()) && (inputCreatures[j].getName().compareTo(pivot.getName()) > 0))) {
                j--;
                comp++;
            }
    ...
    private void swap(int i, int menor) {
        Pokemon aux = inputCreatures[i];
        inputCreatures[i] = inputCreatures[menor];
        inputCreatures[menor] = aux;
    }
}
...
public class Creatures {
    ...
    public static void main(String[] args) {
        ...
        int tam = 0;
        Pokemon[] inputCreatures = new Pokemon[51];

        while (!(id.equals("FIM"))) {
            for (Pokemon p : pokedex) {
                if (p.getId() == Integer.parseInt(id)) {
                    inputCreatures[tam++] = p;
                    break;
                }
            }
        ...
        PartialQuickSort quickSort = new PartialQuickSort(inputCreatures, tam);
        quickSort.sort();
        ...
        String conteudo = "844188" + "\t" + comp + "\t" + mov + '\t' + executionTime + "ms";

        try (BufferedWriter writer = new BufferedWriter(new FileWriter("844188_partialQuickSort.txt"))) {
